...
OLD_BASELINE: previous governance-only proof at d419db3 was valid only for references, source pin and diff scope. NEW_EVIDENCE: this repository's Actions run 34254667212 proves the integration violates the existing scanner gate; run 34255523613 confirms DoneAudit now fails closed as well. SUPERSEDED_DECISION: safe integration is BLOCKED, not VERIFIED. The existing security command is now required in DoneAudit, so earlier narrow green receipts cannot override this regression. Local ignored proof receipts produce additional scanner findings; they are not tracked or published. Findings are redacted; do not copy matched tokens into reports.
